<template>
	<view class="works-swiper">
		<view class="fixed-top">
			<u-status-bar />
			<view class="flex justify-between align-center works-swiper__header"
				:style="{height: $u.addUnit($u.getPx(44))}">
				<view class="ml-3" @click="back">
					<u-icon color="#ffffff" name="arrow-left"></u-icon>
				</view>
				<view class="text-white works-swiper__header-title">{{ currentIndex + 1 }}/{{ list.length }}</view>
		...
		<view class="works-swiper__indicator">
			<u-swiper-indicator :indicatorActiveColor="indicatorActiveColor"
