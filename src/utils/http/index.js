...
http.interceptors.request.use((config) => {
	...
	if (config.custom.isComplete) {
		config.baseURL = BAIDU_YUN;
	}
...
http.interceptors.response.use(async (response) => {
	...
	if (response.config.custom.isComplete) {
		return response.data
	}

	if (response.data.suc != 1) { // 服务端返回的状态码不等于true，则reject()
	...
	if (response.statusCode == 401) {
		...
		} else { //正式环境不在小程序内弹出登录，应该由主程序进行登录
...
function _reqlog(req) {
	if (process.env.NODE_ENV === 'development') {
		console.log("【" + TITLE + "】 地址：" + JSON.stringify(omit(req, ['data', 'params'])))
		if (req.data) {
			console.log("【" + TITLE + "】 请求参数data：" + JSON.stringify(req.data))
		}
		if (req.params) {
			console.log("【" + TITLE + "】 请求参数params：" + JSON.stringify(req.params))
		}
	}
	//TODO 调接口异步写入日志数据库
}
...
function _reslog(res) {
	...
	if (process.env.NODE_ENV === 'development') {
		console.log("【" + TITLE + "】 地址：" + JSON.stringify(omit(res, ['data'])))
