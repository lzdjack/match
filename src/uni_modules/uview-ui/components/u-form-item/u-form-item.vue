<template>
	<view class="u-form-item">
		<view class="u-form-item__body" @tap="clickHandler" :style="[$u.addStyle(customStyle), {
				flexDirection: (labelPosition || parentData.labelPosition) === 'left' ? 'row' : 'column'
			}]">
			<!-- 微信小程序中，将一个参数设置空字符串，结果会变成字符串"true" -->
			<slot name="label">
				<!-- {{required}} -->
				<view class="u-form-item__body__left" v-if="required || leftIcon || label" :style="{
					...
					<view class="u-form-item__body__left__content">
						...
						<text decode class="u-form-item__body__left__content__label" :style="[parentData.labelStyle, {
		...
		<slot name="error">
			<text v-if="!!message && parentData.errorType === 'message'" class="u-form-item__body__right__message"
				:style="{
					marginLeft:  $u.addUnit(parentData.labelPosition === 'top' ? 0 : (labelWidth || parentData.labelWidth))
				}">{{ message }}</text>
		</slot>
		<u-line v-if="borderBottom"
			:color="message && parentData.errorType === 'border-bottom' ? $u.color.error : propsLine.color"
			:customStyle="`margin-top: ${message && parentData.errorType === 'message' ? '5px' : 0}`"></u-line>
	</view>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			this.init()
			this.parentData.labelPosition = this.labelPosition || 'left'
		},
		methods: {
			init() {
				// 父组件的实例
